...
> 核心部分（防超卖、状态机、缓存防击穿、超时关单）已写好，你照着 README 把环境跑通、
> 补上细节，然后**自己动手压测出数据**，这个项目就真正属于你了。
...
- [ ] **JMeter 压测**：1000 并发抢 100 张票，确认恰好成功 100 单、无超卖，截图存进 README
- [ ] 管理端/用户端前端页面（Vue3 或 Thymeleaf）
- [ ] 模拟支付接口返回成功页
- [ ] 入场核销（二维码/验证码）
- [ ] 把这个仓库推到 GitHub，README 配架构图

## 面试必讲三个点

1. 防超卖：Lua 原子预扣 + 乐观锁兜底 + 压测数据
2. 缓存击穿：互斥锁重建
3. 状态机 + 定时关单 + 回补库存
